"""BCI ErrP pipeline verified against the recovered author repository."""
...
class BCIErrPTask:
    name = "bci_errp"
    cache_version = "bci-v3-author-corrected"
    ...
    def prepare(self, data_dir: Path, cache_dir: Path, denoiser, checkpoint_path: Path | None,
        ...
        variant_cache = cache_dir / self.name / f"{self.cache_version}-{denoiser.name}.npz"
        ...
        if denoiser.name == "bandpass":
            # Original total.ipynb: filter each raw epoch, then subtract its baseline.
            signals = denoiser.transform(raw.signals, FS, **kwargs)
            signals -= np.mean(signals[:, :, :20], axis=2, keepdims=True)
        elif denoiser.name == "asr":
            # Original total.ipynb: 1--40 Hz, baseline, then ASR per epoch.
        ...
        dataset = SignalDataset(
            ...
            groups=raw.groups, metadata={
                "source": "SunGj921028/BCI-task",
                "channels": CHANNELS,
                "preprocessing_scope": "epoch",
                "xdawn_test_policy": "train_fit_test_transform",
            },
        )
    ...
    def standardize_features(x_train, x_test):
        # The recovered BCI notebook feeds tangent-space features directly to
        # the classifiers without an additional StandardScaler.
    ...
    def _quick(data: SignalDataset) -> SignalDataset:
        ...
        return SignalDataset(data.signals[indices], data.labels[indices], data.sampling_rate,
            data.class_names, data.primary_metric, data.fixed_train[indices], data.groups[indices], data.metadata)
